/*
 * =========================
 * AUTHOR INFO
 * =========================
 */

import { StringKeyValueType } from '../types';

export const socialLinks: StringKeyValueType = {
  facebook: 'https://www.facebook.com/vatsalsinghkv',
  twitter: 'https://x.com/KMarkhoor',
  github: 'https://github.com/Arbab-Mustafa',
  linkedin: ' https://www.linkedin.com/in/arbabmustafa/',
};

export const author = {
  name: 'Arbab Mustafa',
  email: 'arbabmachi132@gmail.com',
};

export const seoData = {
  title: ' Arbab Mustafa |  Full Stack Engineer',
  description:
    'Vatsal Singh is a front-end developer who specializes in building (and occasionally designing) exceptional visual interfaces.',
  author: author.name,
  image:
    ' https://www.arbabmustafa.com/_next/image?url=%2Fprofile.jpeg&w=256&q=95',
  url: 'https://arbabmustafa.com',
  keywords: [
    'Arbab',
    'Arbab Mustafa',
    '@Arbab-Mustafa',
    'arbabmustafa',
    '@arbabmustafa',
    'Portfolio',
    'Arbab Portfolio ',
    'Arbab Mustafa Portfolio',
    '@mustafaarbab',
    'Software Engineer',
    'frontend developer',
    'fullstack developer',
    'react developer',
    'reactjs developer',
    'web developer',
    'javascript developer',
    'typescript developer',
    'react developer',
    'software engineer',
    'uet lahore',
    'uet lahore student',
    'uet lahore graduate',
    'uet lahore alumni',
    'uet lahore software engineer',
    'uet lahore web developer',
    'uet lahore frontend developer',
    'uet lahore fullstack developer',
    'uet lahore react developer',
    'uet lahore reactjs developer',
    'uet lahore javascript developer',
    'uet taxila',
    'uet taxila student',
    'uet taxila graduate',
    'uet taxila alumni',
    'uet taxila software engineer',
    'uet taxila web developer',
    'uet taxila frontend developer',
    'uet taxila fullstack developer',
    'uet taxila react developer',
    'uet taxila reactjs developer',
    'uet taxila javascript developer',
    'uet',
  ],
};
